...
def get_watchlists(self):
    ...
    results = self.call_api(method="GET", url_suffix=url_suffix, use_auth=True)[
	"watchlists"
    ]["watchlist"]
...
def new_watchlist(self, name, symbols=[]):
	...
	use_auth=True,
	data={"id": str(name), "symbols": ",".join(symbols)},
...
def watchlist(self, name):
    ...
    results = [entry["instrument"]["sym"] for entry in results]
...
def delete_watchlist(self, name):
    ...
    results = self.call_api(method="DELETE", url_suffix=url_suffix, use_auth=True,)[
	"watchlists"
    ]["watchlist"]
