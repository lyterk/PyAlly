...
def get_strike_prices(self, symbol=""):
	...
	method="GET", url_suffix="market/options/strikes.json", data={"symbol": symbol}
    )["prices"]["price"]
...
def search_options(self, symbol="", query="", fields=""):
    ...
    data = {"symbol": symbol, "query": fmt_query, "fids": ",".join(fields)}

    results = self.call_api(
	method="POST", url_suffix="market/options/search.json", data=data
    )["quotes"]["quote"]
...
def options_chain(self, symbol="", direction="c", within_pct=4.0, exp_date=""):
    ...
    # Safety first!
    if (
	not utils.check(symbol)
	or not utils.check(direction)
	or not utils.check(exp_date)
    ):
	return []
    ...
    results = self.search_options(symbol=symbol, query=fmt_query)
