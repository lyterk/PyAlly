...
def call_api(
    ...
    if full_output:
	return {"response": r, "request": utils.pretty_print_POST(req)}
    else:
	return r
...
def get_accounts(self):
    ...
    acnts = self.call_api(method="GET", url_suffix="accounts.json")["accounts"][
	"accountsummary"
    ]
...
def get_holdings(self, account=None, verbose=False):
	...
	method="GET", url_suffix="accounts/" + str(account) + "/holdings.json"
    )["accountholdings"]
...
def get_quote(self, symbols, fields=[]):
	...
	method="POST", url_suffix="market/ext/quotes.json", data=req_params
    )["quotes"]["quote"]
...
def account_history(self, account=None, type_="all", range_="all"):
	...
	account = self.params["account"]
    # Add parameters
    data = {"range": range_, "transactions": type_}

    results = self.call_api(
	url_suffix="accounts/" + str(account) + "/history.json", method="GET", data=data
    )
...
def order_history(self, account=None, verbose=False):
    ...
    results = self.call_api(
	method="GET", url_prefix="accounts/" + str(account) + "/orders.json"
    )
...
def timesales(
    ...
    results = self.call_api(method="GET", url_suffix=url_suffix, data=data)
...
def market_clock(self):
    ...
    results = self.call_api(method="GET", url_suffix=url_suffix, use_auth=False)

    x = {k: v for k, v in results.items() if k in ("message", "status")}
    return x
...
def api_status(self):
    ...
    return {k: v for k, v in results.items() if k in ("time", "error")}
...
def get_member(self):
	...
	"""
    results = self.call_api(method="GET", url_suffix="member/profile.json")["userdata"]
    ...
    return {**x, **{k: v for k, v in results["account"].items()}}
